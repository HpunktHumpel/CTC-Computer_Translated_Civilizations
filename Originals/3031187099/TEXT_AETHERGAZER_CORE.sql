...
('LOC_DISTRICT_PRESERVE_AETHERGAZER_DESCRIPTION','zh_Hans_CN','圣树神域特色区域，替代保护区，初始解锁，造价更低且不会涨价，可以无视人口需求建造。[NEWLINE]弥特尔系统会为周围单元格会+2[ICON_Gold]金币以及+2 [ICON_Production]生产力（即使是未建成），建成后周围单元格+3魅力。[NEWLINE]古树林建成后，周围魅力值为普通或普通以上的单元格+2[ICON_Food]食物+2 [ICON_Faith]信仰+2[ICON_Science]科研。[NEWLINE]避难所建成后，周围魅力为普通或普通以上的单元格+2[ICON_Production]生产力+2[ICON_Gold]金币+2[ICON_Science]科研。[NEWLINE]上述加成，单元格无论是否存在改良设施均能获得。'),
...
('LOC_IMPROVEMENT_PLANTATION_AETHERGAZER_DESCRIPTION','zh_Hans_CN','特色改良设施，仅有尼罗神域的修正者领袖能够建造，+2[ICON_Food]食物+2[ICON_Gold]金币+1[ICON_Science]科技+1[ICON_Culture]文化+5[ICON_Housing]住房+2[ICON_Amenities]宜居度，研究相关科技后可获得更多产出，允许在没有资源的泛滥平原、绿洲、沼泽上建造，无法建造在另一个科技种植园旁'),
...
('LOC_TRAIT_CIVILIZATION_AETHERGAZER_YELLOW_DESCRIPTION','zh_Hans_CN','沙漠平原和沙漠丘陵+2[ICON_Food]食物+2[ICON_Production]生产力+1[ICON_Science]科研，草原泛滥平原+2[ICON_Production]生产力+3魅力，平原泛滥平原+1[ICON_Food]食物+1[ICON_Production]生产力+3魅力，沙漠泛滥平原+2[ICON_Gold]金币+3魅力，树林提供-3魅力，允许农田建在沙漠平原上，每位[ICON_Citizen]公民+1[ICON_Gold]金币');
/*
('','zh_Hans_CN',''),
('','zh_Hans_CN',''),
('','zh_Hans_CN',''),
