...
('LOC_TRAIT_LEADER_TUOER_TYPEONE_AETHERGAZER_DESCRIPTION','zh_Hans_CN','除了拥有轰雷·托尔的领袖技能外，城市内单元格的雨林和森林的单元格+2[ICON_Gold]金币+2[ICON_Faith]信仰，所有单位上船下船时[ICON_Movement]无移动力消耗。研究{LOC_TECH_SHIPBUILDING_NAME}后，所有单位拥有进入海岸、海洋单元格的能力。当前政体的所有军事政策槽位均转为通配符槽位，有两个或以上[ICON_GreatWork_Sculpture]巨作槽位的奇观和建筑自动获得[ICON_ThemeBonus_Active]主体化。所有[ICON_GreatWork_Sculpture][ICON_GreatWork_Portrait][ICON_GreatWork_Landscape][ICON_GreatWork_Religious]类型的艺术品和[ICON_GreatWork_Writing]著作+3[ICON_Food]食物+3[ICON_Production]生产力+3[ICON_Gold]金币，宫殿+2[ICON_GreatWork_Sculpture]巨作槽位，纪念碑+3[ICON_GreatWork_Sculpture]巨作槽位+6[ICON_GreatArtist]大画家点数，所有城市建立时带有一个额外的市中心建筑（远古时代是纪念碑）'),
--托尔外交
--同意蓝脸
